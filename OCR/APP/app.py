...
def scandoc():
    ...
    if request.method == "POST":
        ...
        four_point,size = docscan.document_scanner(upload_image_path)
        ...
        if four_point is None:
            ...
            points = utils.array_to_json_format(four_point.reshape((4,2)))
...
def transform():
    try:
        ...
        arr = np.array(points)
        magic_color  = docscan.calibrate_to_original_size(arr.reshape((4,1,2)))
...
def prediction():
    ...
    image_bb,results = predictions.getPredictions(image)
